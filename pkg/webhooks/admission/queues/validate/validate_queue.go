...
func validateSiblingsSum(queue, parent *schedulingv1beta1.Queue, siblings []*schedulingv1beta1.Queue) error {
	...
	if err := validateResourceLimit(totalGuarantee, "guarantee", parent.Spec.Guarantee.Resource); err != nil {
		return fmt.Errorf("parent queue %s validation failed: %v", parent.Name, err)
	}

	// Validate deserved sum
	if err := validateResourceLimit(totalDeserved, "deserved", parent.Spec.Deserved); err != nil {
		return fmt.Errorf("parent queue %s validation failed: %v", parent.Name, err)
	}
...
// validateResourceLimit checks if children's sum exceeds parent's guarantee/deserved limit
// If the guarantee/deserved parameter of the queue is not configured, the default value is 0.
func validateResourceLimit(childrenSum *api.Resource,
